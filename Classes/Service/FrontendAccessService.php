...
{
    private const EXTENSION_KEY = 'ppl_deepl_v2_translate';
    private const COOKIE_NAME = 'ppl_deepl_v2_login';
    private const COOKIE_TTL = 86400;
    private const LOGIN_ATTEMPT_LIMIT = 5;
    private const LOGIN_ATTEMPT_WINDOW = 900;
    private const LOGIN_LOCK_TTL = 900;
    ...
    public function buildAccessResponse(
        ...
        $submittedUsername = trim((string)($body['user'] ?? ''));
        $loginAttemptFailed = false;

        if ($isPplLoginAttempt) {
            if (!$this->isSameOriginRequest($request) || $this->isLoginRateLimited($request, $submittedUsername)) {
                $loginAttemptFailed = true;
            } else {
                $authenticatedUser = $this->authenticateSubmittedLogin($body, $allowFrontendUsers, $allowBackendUsers);
                if ($authenticatedUser !== null) {
                    $this->clearLoginRateLimit($request, $submittedUsername);
                    $returnUrl = $this->getSubmittedSafeReturnUrl($request) ?? $this->buildSafeReturnUrl($request);
                ...
                $this->recordFailedLoginAttempt($request, $submittedUsername);
                $loginAttemptFailed = true;
            }
        }
        ...
        return new HtmlResponse(
            $this->renderInlineLogin($request, $loginAttemptFailed, $submittedUsername)
        );
    }
    ...
    private function getTypoScriptFallbackValue(string $settingName): string
    {
        ...
        if (!preg_match('/^plugin\\.tx_ppldeeplv2translate\\.settings\\.' . preg_quote($settingName, '/') . '\\s*=\\s*(.*?)\\s*$/m', $contents, $matches)) {
            return '';
        }
    ...
    private function isLoginRateLimited(ServerRequestInterface $request, string $username): bool
    {
        $state = $this->getLoginAttemptState($request, $username);

        return (int)($state['lockedUntil'] ?? 0) > time();
    }

    private function recordFailedLoginAttempt(ServerRequestInterface $request, string $username): void
    {
        $now = time();
        $filePath = $this->getLoginAttemptFilePath($request, $username);
        ...
        try {
            ...
            $firstAttempt = (int)($state['firstAttempt'] ?? 0);
            $attempts = (int)($state['attempts'] ?? 0);

            if ($firstAttempt <= 0 || $firstAttempt + self::LOGIN_ATTEMPT_WINDOW < $now) {
                $firstAttempt = $now;
                $attempts = 0;
            }

            $attempts++;
            $lockedUntil = $attempts >= self::LOGIN_ATTEMPT_LIMIT ? $now + self::LOGIN_LOCK_TTL : 0;
    ...
    private function clearLoginRateLimit(ServerRequestInterface $request, string $username): void
    {
        $filePath = $this->getLoginAttemptFilePath($request, $username);
        if (is_file($filePath)) {
            @unlink($filePath);
    ...
    private function getLoginAttemptState(ServerRequestInterface $request, string $username): array
    {
        $filePath = $this->getLoginAttemptFilePath($request, $username);
        if (!is_file($filePath)) {
            return [];
    ...
    private function getLoginAttemptFilePath(ServerRequestInterface $request, string $username): string
    {
        return Environment::getVarPath()
            . '/transient/' . self::EXTENSION_KEY . '/login-attempts/'
            . $this->buildLoginAttemptCacheKey($request, $username) . '.json';
    }

    private function buildLoginAttemptCacheKey(ServerRequestInterface $request, string $username): string
    {
        ...
        return 'ppldeepllogin_' . substr(hash('sha256', self::EXTENSION_KEY . '|' . $this->getClientIp($request) . '|' . $username), 0, 48);
    }

    private function getClientIp(ServerRequestInterface $request): string
    {
        $serverParams = $request->getServerParams();
        $clientIp = trim((string)($serverParams['REMOTE_ADDR'] ?? ''));
        if ($clientIp === '') {
            $clientIp = trim((string)GeneralUtility::getIndpEnv('REMOTE_ADDR'));
        }
    ...
    private function decodeLoginCookie(string $cookie): ?array
    {
        ...
        if (!is_array($data)
            || (string)($data['purpose'] ?? '') !== self::EXTENSION_KEY
